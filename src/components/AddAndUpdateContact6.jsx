...
const AddAndUpdateContact6 = ({ isOpen, onClose, isUpdate, contact }) => {
  ...
  const addContact = async (contact) => {   //inserção de dados
    ...
    const userEmail = user.email;
      ...
      const contactRef = collection(db, "mems");
  ...
  const updateContact = async (contact, id) => {
    try {
      const contactRef = doc(db, "mems", id);
...
export default AddAndUpdateContact6;
