...
const AddAndUpdateContact7 = ({ isOpen, onClose, isUpdate, contact }) => {
  ...
  const addContact = async (contact) => {   //inserção de dados
    ...
    const userEmail = user.email;
      ...
      const contactRef = collection(db, "chicotes");
  ...
  const updateContact = async (contact, id) => {
    try {
      const contactRef = doc(db, "chicotes", id);
...
export default AddAndUpdateContact7;
