...
const AddAndUpdateContact4 = ({ isOpen, onClose, isUpdate, contact }) => {
  ...
  const addContact = async (contact) => {   //inserção de dados
    ...
    const userEmail = user.email;
      ...
      const contactRef = collection(db, "UMR");
  ...
  const updateContact = async (contact, id) => {
    try {
      const contactRef = doc(db, "UMR", id);
...
export default AddAndUpdateContact4;
